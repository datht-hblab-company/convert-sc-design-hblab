...
test("package.json includes publish-ready npm metadata", () => {
  ...
  assert.equal(packageJson.name, "convert-sc-design-hblab");
  assert.equal(packageJson.version, "0.3.0");
  assert.equal(packageJson.skills.name, "convert-sc-design-hblab");
...
test("skill examples and docs mention gid-based sheet selection", () => {
  const skillDoc = fs.readFileSync(
    path.join(__dirname, "..", "skills", "convert-sc-design-hblab", "SKILL.md"),
    "utf8"
  );
  const jsonExample = fs.readFileSync(
    path.join(__dirname, "..", "skills", "convert-sc-design-hblab", "examples", "read-sheet-json.sh"),
    "utf8"
  );
  const tableExample = fs.readFileSync(
    path.join(__dirname, "..", "skills", "convert-sc-design-hblab", "examples", "read-sheet-table.sh"),
    "utf8"
  );
